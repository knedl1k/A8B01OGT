...
Uvažme lineární prostor $\mathbb{S}^n = \bc{A \in \mathbb{M}_n (\R) \mid A^ T = T}$ reálných symetrických $n \times n$
matic se skalárním součinem $\langle A B \rangle _{\mathbb{S}_n} = Tr (AB)$.
\begin{enumerate}[(a)]
    \item Ukažte, že $
    \begin{bmatrix}
        \begin{pmatrix}
        ...
        \end{pmatrix}
    \end{bmatrix}$
    je ortonormální báze na $\mathbb{S}^2$.
    \item Ukažte, že zobrazení
    \[ \varphi:
    \begin{bmatrix}
        a & b \\
        b & c
    \end{bmatrix} \in \mathbb{S}^2 \mapsto
    \begin{bmatrix}
        a \\
        \sqrt{2}b \\
        c
    \end{bmatrix} \in \R^3
    \]
    je isomorfismus lineárního prostoru $\mathbb{S}^2$ na $\R^3$ zachovávající skalární součin (tj. \\ $\langle A, B \rangle _{\mathbb{S}_2} = \langle \varphi(A), \varphi(B) \rangle$ pro všechna $A,B \in
    \mathbb{S}^2$, kde $\langle \dots \rangle$ je standardní skalární součin na $\R^3$)
    \item Zobecněte výsledky bodů (a) a (b) do prostoru $\mathbb{S}^2 na \R^2$ zachovávající skalární součin.
    \item Ať $\mathbb{S}^2_+$ je množina všech reálných symetrických $2 \times 2$ matic, které jsou navíc positivně
    semidefinitní. Ukažte, že jestliže $\varphi$ je zobrazení z bodu (b), pak
    \[\varphi(\mathbb{S}^2_+) = \bc{(x,y,z)^T \in \R^3 \mid x \geq 0, z \geq 0, 2xz - y^2 \geq 0} \text{.}\]
\end{enumerate}

\subsection{Bijekce mezi dvěma optimalisačními úlohami}
Je dána úloha
\begin{align*}
    \text{minimalisujte } & \langle X, A \rangle _{\mathbb{S}_2} \\
    \text{za podmínek }   & \langle X, \bb{1} \rangle _{\mathbb{S}_2} = 2, \\
                          & X \in \mathbb{S}_+^2,
\end{align*}
kde $A = 
    \begin{bmatrix}
...
\begin{align*}
    \text{minimalisujte } & \langle X, A \rangle _{\mathbb{S}_2} \\
    \text{za podmínek }   & \langle X, B \rangle _{\mathbb{S}_2} = 0, \\
                          & \langle X, \bb{1} \rangle _{\mathbb{S}_2} = 1, \\
                          & X \in \mathbb{S}_+^2,
\end{align*}
kde $A = 
    \begin{bmatrix}
...
Ať $A \in \mathbb{M}_n (\R)$.
\begin{enumerate}[(a)]
    \item Ukažte, že $\langle Ax, y \rangle = \langle x, A^T y \rangle$ pro všechna $x, y \in \R^n$.
    \item Ukažte, že existují matice $B, C \in \mathbb{M}_n (\R)$ takové, že $B^T = B$, $C^T = -C$ a $A = B + C$. Jsou 
    matice $B$ a $C$ určeny jednoznačně?
    \item Ukažte, že existuje symetrická matice $B \in \mathbb{M}_n (\R)$ taková, že $\langle Ax, x \rangle = 
    \langle Bx, x \rangle$.
\end{enumerate}
...
\begin{enumerate}[(a)]
    ...
    \item $f(x) = \langle Ax, x \rangle$, kde $A \in \mathbb{M}_n (\R)$. Určete také $\nabla f(x)$ a $\nabla^2 f (x)$ za
