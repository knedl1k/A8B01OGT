...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=1]
        \draw[->, thick] (-0.5, 0) -- (5, 0) node[below left] {$y$};
        \draw[->, thick] (0, -0.5) -- (0, 4) node[below left] {$z$};

        \draw[blue, thick, domain=0:4.5] plot (\x, {0.4*(\x-2)^2 + 1}) node[above  ] {$f(y)$};

        % --- point of contact (x, f(x)) ---
        \pgfmathsetmacro{\xval}{3.5}
        \pgfmathsetmacro{\yval}{0.4*(\xval - 2)^2 + 1}
        
        \draw[dashed] (\xval, 0) node[below] {$x$} -- (\xval, \yval);
        \draw[dashed] (0, \yval) node[left] {$f(x)$} -- (\xval, \yval);

        % --- tangent ---
        % Equation of a tangent: z - f(x) = f'(x)*(y - x)
        \pgfmathsetmacro{\m}{0.8*(\xval - 2)} % tangent direction
        \draw[orange, thick] 
        ({\xval - 2.5}, {\yval - 2.5*\m}) -- ({\xval + 1.5}, {\yval + 1.5*\m});
        
        \node at (2.5, -0.7) [orange]{\large$z = f(x) + \langle \nabla f(x), y - x \rangle$};
